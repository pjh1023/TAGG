...
class ExtraFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        x_cycles, y_cycles = self.ncycles(noisy_data)       # (bs, n_cycles)    # error here
...
class KNodeCycles:
    ...
    def k4_cycle(self):
        ...
        # diag_a2 = batch_diagonal(self.k2_matrix)
        # c4 = diag_a4 + diag_a2 - diag_a2*diag_a2
        return (c4 / 2).unsqueeze(-1).float(), (torch.sum(c4, dim=-1) / 8).unsqueeze(-1).float()

    def k5_cycle(self):
        diag_a5 = batch_diagonal(self.k5_matrix)
        # diag_a3 = batch_diagonal(self.k3_matrix)
        ...
        # c5 = diag_a5 - diag_a3 - (self.k1_matrix.sum(dim=2)-2)*diag_a3
