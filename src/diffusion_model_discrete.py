...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        self.train_loss = TrainLossDiscrete(self.cfg.model)
    ...
    def training_step(self, data, i):
        ...
        pred = self.forward(noisy_data, extra_data, node_mask, data)
    ...
    def validation_step(self, data, i):
        self.data = data        # kept so that on_validation_epoch_end can pass a data object to sample_batch()
        dense_data, node_mask = utils.to_dense(data.x, data.edge_index, data.edge_attr, data.batch)
        dense_data = dense_data.mask(node_mask)
        noisy_data = self.apply_noise(dense_data.X, dense_data.E, data.y, node_mask)
        extra_data = self.compute_extra_data(noisy_data)
        pred = self.forward(noisy_data, extra_data, node_mask, data)
        nll = self.compute_val_loss(pred, noisy_data, dense_data.X, dense_data.E, data.y, data, node_mask, test=False)
        return {'loss': nll}
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # Averaged homological feature vector mu_{G'} over all training graphs (Sec. 4.4),
        # used as the condition for the reverse process where no reference graph exists.
        mean_vectorization = []
        for temp_d in self.trainer.datamodule.train_dataset:
            mean_vectorization.append(temp_d.vectorization)

        mean_vectorization = torch.cat(mean_vectorization, dim=0)
        mean_vectorization = torch.mean(mean_vectorization, dim=0, keepdim=True)    # (1, dv)
        ...
        if self.val_counter % self.cfg.general.sample_every_val == 0:
            ...
            while samples_left_to_generate > 0:
                ...
                self.data.vectorization = mean_vectorization.expand(to_generate, -1).to(self.device)
    ...
    def test_step(self, data, i):
        ...
        extra_data = self.compute_extra_data(noisy_data)
        pred = self.forward(noisy_data, extra_data, node_mask, data)
        nll = self.compute_val_loss(pred, noisy_data, dense_data.X, dense_data.E, data.y, data, node_mask, test=True)
        return {'loss': nll}
    ...
    def on_test_epoch_end(self) -> None:
        ...
        while samples_left_to_generate > 0:
            ...
            samples.extend(self.sample_batch(id, to_generate, num_nodes=None, save_final=to_save,
                                             keep_chain=chains_save, number_chain_steps=self.number_chain_steps, data=self.data))
            id += to_generate
            samples_left_to_save -= to_save
            samples_left_to_generate -= to_generate
            chains_left_to_save -= chains_save
    ...
    def reconstruction_logp(self, t, X, E, node_mask, data):
        ...
        extra_data = self.compute_extra_data(noisy_data)
        pred0 = self.forward(noisy_data, extra_data, node_mask, data)
    ...
    def compute_val_loss(self, pred, noisy_data, X, E, y, data, node_mask, test=False):
       ...
       """
        ...
        prob0 = self.reconstruction_logp(t, X, E, node_mask, data)
    ...
    def forward(self, noisy_data, extra_data, node_mask, data):
        X = torch.cat((noisy_data['X_t'], extra_data.X), dim=2).float()
        E = torch.cat((noisy_data['E_t'], extra_data.E), dim=3).float()
        y = torch.hstack((noisy_data['y_t'], extra_data.y)).float()
        return self.model(X, E, y, node_mask, data)

    @torch.no_grad()
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
                     save_final: int, data, num_nodes=None):
        """
        :param batch_id: int
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            # Sample z_s
            sampled_s, discrete_sampled_s = self.sample_p_zs_given_zt(s_norm, t_norm, X, E, y, node_mask, data)
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, data):
